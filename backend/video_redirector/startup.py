...
async def scheduled_file_id_validation():
    ...
    while True:
        try:
            ...
            next_run = now_kyiv.replace(hour=13, minute=57, second=0, microsecond=0)
            ...
            logger.debug(f"🕒 Next file ID validation scheduled for: {next_run.strftime('%Y-%m-%d %H:%M:%S %Z')}")
            logger.info(f"⏰ Waiting {seconds_until_next:.0f} seconds until next validation...")
            
            # Wait until next run time
            await asyncio.sleep(seconds_until_next)
            
            logger.debug("🕒 Starting scheduled file ID validation...")
            stats = await validate_all_file_ids()
            logger.debug(f"✅ Scheduled file ID validation completed: {stats}")
