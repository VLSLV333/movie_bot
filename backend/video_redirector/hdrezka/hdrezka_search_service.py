...
async def search_hdrezka(query: str, fallback_query: str) -> list[dict]:
    """
    ...
    if not results:
        encoded_fallback_query = quote(fallback_query)
        url_2 = f"https://hdrezka.ag/search/?do=search&subaction=search&q={encoded_fallback_query}"
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Safari/537.36",
            "Accept": "*/*",
            "Origin": "https://hdrezka.ag",
        ...
        async with ClientSession() as session:
            async with session.get(url_2, headers=headers) as resp:
                html = await resp.text()
