...
async def change_language_mirror_handler(query: types.CallbackQuery):
    ...
    if query.message is not None:
        lang_selection_msg = await query.message.answer(
            gettext(SELECT_LANGUAGE_FOR_MOVIES),
        ...
        await query.answer(gettext(ERROR_CANNOT_SHOW_LANGUAGE_OPTIONS))
        return None
        ...
        async with ClientSession() as session:
            async with session.put(
                ...
                if resp.status == 200:
                    ...
                    await query.message.answer(gettext(LANGUAGE_UPDATED_SUCCESSFULLY).format(language=language_display))
                    return

                else:
                    logger.error(f"[User {user_id}] Failed to update movies language: {resp.status}")
                    if query.message is not None:
                        await query.message.answer(gettext(ERROR_UPDATE_LANGUAGE_FAILED))
